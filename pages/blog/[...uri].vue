<template>
    <div id="post" v-if="blogPost">
      <section class="postBlog-info-section flex flex-row py-20 px-10 w-11/12 mx-auto rounded-2xl print:py-2 print:px-0 print:w-full">
        ...
        <div class="flex flex-col w-2/5">
          <NuxtImg v-if="blogPost.featuredImage" class="m-auto h-60 w-auto border rounded-2xl transition-all duration-300 ease-in-out shadow-lg mb-4" :src="blogPost.featuredImage.node.sourceUrl" :alt="blogPost.featuredImage.node.altText" />
        </div>
      </section>
      <!-- Sezione indice -->
      <section class="postBlog-index-section flex flex-col py-20 px-10 w-11/12 mx-auto mt-4 rounded-2xl">
      ...
      <section v-for="(section, index) in sections"
               :class="['post-section flex flex-col py-20 px-10 w-11/12 mx-auto rounded-2xl mt-4 print:py-2 print:px-0 print:w-full', section.className]"
               :id="'section' + (index + 1)"
               :key="section.heading">
    ...
    <div v-else-if="loading" class="flex flex-row py-20 px-10 w-11/12 mx-auto rounded-2xl">
    ...
    <div v-else class="flex flex-row py-20 px-10 w-11/12 mx-auto rounded-2xl">
      <p class="text-center mx-auto">Nessun post trovato.</p>
    </div>
  </template>
  ...
  const { result, loading, error } = useQuery(FETCH_BLOG_POST_BY_SLUG, { slug: slug.value });
  
  watch(result, (newVal) => {
    console.log("Query result:", newVal);
    console.log("Blog post loaded:", newVal?.blogPostBy);
    if (newVal?.blogPostBy) {
      processBlogPost(newVal.blogPostBy);
    }
  }, { deep: true });
  
  const blogPost = computed(() => result.value?.blogPostBy || {});
  ...
  const processBlogPost = (post) => {
    if (post && post.content) {
      console.log("Blog post fully loaded:", post);
      ...
      readingTime.value = Math.ceil(post.content.split(/\s+/).length / 200); // Calcolo del tempo di lettura
