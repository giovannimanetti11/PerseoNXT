<template>
  ...
  const { result, loading, error } = useQuery(FETCH_ALL_GLOSSARY_TERMS, {}, { client: apolloClient });
  
  watchEffect(() => {
    if (result.value && result.value.glossaryTerms) {
      console.log('Query result:', result.value.glossaryTerms.nodes);
      posts.value = result.value.glossaryTerms.nodes.map(post => ({
        objectID: post.id,
        title: post.title,
        slug: post.slug,
  ...
  const updateAlgoliaIndex = async () => {
    ...
    try {
      const response = await algoliaClient.value.initIndex('glossary').partialUpdateObjects(posts.value, { createIfNotExists: true });
