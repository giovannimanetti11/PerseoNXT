<template>
  ...
  <div v-else-if="post.data" id="post">
    ...
    const fetchPost = async () => {
      ...
      const { result, loading, error } = useQuery(FETCH_POST_BY_SLUG, { slug });
      ...
      watch([result, loading, error], async ([newResult, newLoading, newError]) => {
        if (newError) {
          ...
          if (post.data.additionalImages) {
            post.additionalImagesFiltered = post.data.additionalImages.filter(img => img && img.sourceUrl);
          } else {
            post.additionalImagesFiltered = [];
          }
          ...
          post.error = new Error('Post not found');
        }
      });
    };
