<template>
  <section class="py-10 mt-6 w-11/12 mx-auto rounded-2xl section-blog flex flex-col xl:flex-row"> 
    <!-- Left column -->
    ...
    <div class="w-full xl:w-3/5">
      <div class="w-full flex flex-col xl:flex-wrap xl:justify-around">
        <div v-for="(post, index) in posts" :key="post.uri" @click="toggleActive(post.uri)"
          :class="['blog-card w-11/12 xl:w-7/12 flex flex-col xl:flex-row rounded-2xl shadow transition-all m-auto my-2 xl:m-4 cursor-pointer', 
            post.uri === activePost ? 'active bg-celeste text-white' : 'bg-white shadow text-black hover:bg-celeste hover:text-white']"
