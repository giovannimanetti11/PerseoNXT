import { ref, h, resolveComponent, type VNode } from 'vue'

type SocialPlatform = 'facebook' | 'twitter' | 'linkedin' | 'whatsapp' | 'telegram'

// Composable function to manage social sharing and link copying
export function useShare() {
  const isShareModalOpen = ref<boolean>(false)
  const currentUrl = ref<string>('')
  const showLinkCopyFeedback = ref<boolean>(false)
  ...
  const shareOnSocial = (platform: SocialPlatform): void => {
  ...
  const renderShareModal = (): VNode | null => {
