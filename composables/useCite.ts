...
type CitationStyle = 'MLA' | 'APA' | 'Wikipedia'

// Composable function to manage citations with dynamic styles and modal control
export function useCite(initialTitle?: string, initialAuthorName?: string, initialPublishDate?: string) {
  const nuxtApp = useNuxtApp()
  
  // Setup initial state with default values
  const title = ref<string>(initialTitle || 'Titolo non disponibile')
  const authorName = ref<string>(initialAuthorName || 'Editors of Wikiherbalist')
  const publishDate = ref<string>(initialPublishDate || new Date().toISOString())

  // State for modal visibility and citation formatting
  const isCiteModalOpen = ref<boolean>(false)
  const selectedCitationStyle = ref<CitationStyle>('MLA') // Default citation style
  const showCitationCopyFeedback = ref<boolean>(false)

  const openCiteModal = (): void => {
    isCiteModalOpen.value = true
  }

  const closeCiteModal = (): void => {
  ...
  const citationParts = computed((): CitationPart[] => {
  ...
  const renderCiteModal = (): VNode | null => {
      ...
      h('div', {
        ...
        h('div', { class: 'mt-4' }, [
          ...
          h('select', {
            ...
            onChange: (e: Event) => selectedCitationStyle.value = (e.target as HTMLSelectElement).value as CitationStyle,
  ...
  return {
    ...
    copyCitation,
    renderCiteModal,
    updateTitle: (newTitle?: string): void => { title.value = newTitle || 'Titolo non disponibile' },
    updateAuthorName: (newAuthorName?: string): void => { authorName.value = newAuthorName || 'Editors of Wikiherbalist' },
    updatePublishDate: (newPublishDate?: string): void => { publishDate.value = newPublishDate || new Date().toISOString() }
  }
}
